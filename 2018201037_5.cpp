...
//    priority_queue<int, vector<int>> maxheap;
    heap maxheap = heap(MAX_HEAP);
    // Contains larger numbers
//    priority_queue<int, vector<int>, greater<>> minheap;
    heap minheap = heap(MIN_HEAP);
    ...
    while (n--) {
        ...
        // If the difference b/w the sizes is more than one
        // then pop() from the bigger one and add to smaller one
        ...
        if (minheap.size() == maxheap.size()) {
            ...
            printf("%.1f\n", 1.0 * (minheap.size() > maxheap.size() ? minheap.top() : maxheap.top()));
        }
    }
    return 0;
}